...
#Functions to assist with search cursor from https://gis.stackexchange.com/questions/175781/selecting-rows-in-a-layer-using-arcpy-searchcursor
...
#Define directory holding DEM data, tiles, and point features
inDir = "D:/for_caleb/"

#Tiles to process
tiles = inDir + "tiles.shp"

#Pnts to process
pnts = inDir + "pnt_test.shp"

#Define input DEM
demData = inDir + "dem2.tif"

#Define directory to store outputs and scratch data
outDir = inDir + "out/"
scratchDir = inDir + "scratch"

#Set tile buffer distance
buffDist = "100 Meters"

#Tile Unique ID Field
tileID = "TileNum"
...
#Name Output Points
outPnts = inDir + "pnts/" + "point_set.shp"

#Snap all output to DEM
env.snapRaster = demData
...
#Prep to iterate over each tile using search cursor 

fieldList = getFieldList(tiles)

fieldIndex = returnFieldIndex(tiles, tileID)

arcpy.management.MakeFeatureLayer(tiles, "myTiles")

#Iterate over tiles
with arcpy.da.SearchCursor(tiles, (fieldList)) as cursor:
    for row in cursor:
        #Define where clause to select row based on ID column
        whereClause = createWhereClause("myTiles", tileID, [row[fieldIndex]])
        
        # Clear selection
        arcpy.SelectLayerByAttribute_management("myTiles", "CLEAR_SELECTION")
        
        #Make new selection
        arcpy.SelectLayerByAttribute_management("myTiles", "NEW_SELECTION", whereClause)
       
        # Make temp file of just selected feature
        tile = arcpy.management.CopyFeatures("myTiles")
        
        #Buffer tile by defined distance
        tileB = arcpy.Buffer_analysis(tile, buffer_distance_or_field=buffDist)
        
        # Extract DEM within tile buffer
        demIn = ExtractByMask(demData, tileB)
        
        # Extract points within tile 
        pntsIn = arcpy.Clip_analysis(pnts, tile)
        ...
        #List all rasters in Working Directory
        rlst = arcpy.ListRasters()
        ...
        #Make list of lists for input raster and associated column name
        def merge(list1, list2):

            merged_list = [[list1[i], list2[i]] for i in range(0, len(list1))]
            return merged_list

        rExt = merge(rlst, r_names)

        #Extract terrain derivatives at points
        ExtractMultiValuesToPoints(pntsIn, rExt, "NONE")
        
        #If first iteration, make new pont layer
        #If not, append to new point layer
        if row[fieldIndex] == 1 :
            arcpy.CopyFeatures_management(pntsIn, outPnts)
        else :
            arcpy.Append_management([pntsIn], outPnts)