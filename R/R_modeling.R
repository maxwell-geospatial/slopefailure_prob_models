...
#Filter just classes of interest
setc <- filter(set, class == "not" | class == "slopeD")

#Define all categorical variables to factors
...
#Remove rows with missing data and drop unused levels
setc[setc == "-9999"] <- NA
...
set.seed(42)
test <- setd %>% group_by(class) %>% sample_n(750, replace=FALSE)
train <- dplyr::setdiff(setd, test)
...
predict(stack, all.model, type="prob", index=2, na.rm=TRUE, 
        ...
        filename="NAME OUTPUT RASTER FILE")

#Read in and visualize prediction
raster_result <- raster("READ IN RASTER OUTPUT")
...
#Save model to disk
saveRDS(all.model, "NAME YOUR MODEL FILE (MUST BE IN .rds FORMAT")
