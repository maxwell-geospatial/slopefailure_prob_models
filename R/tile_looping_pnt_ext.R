...
#Read DEM
dem <- "D:/for_caleb/dem2.tif"
demData <- raster(dem)

#Read in point data
pnts <- st_read("D:/for_caleb/pnt_test.shp")

#Read in tiles
tiles <- st_read("D:/for_caleb/tiles.shp")

#Define output directory
#Will need to set your own paths
outDir <- "D:/for_caleb/r_out/"
pntDir = "D:/for_caleb/pnts_r/"
...
for(tNum in 1:nrow(tiles)){
  ...
  pntsSub <- st_intersection(pnts, tileIn)
  ...
  writeRaster(topo_stack, paste0(outDir, "r_topo_stack.img"), overwrite=TRUE)
  
  pntsExt <- extract(topo_stack, pntsSub)
  
  names(pntsExt) <- c("slp","diss1","diss2","diss3","sar","srr1",
                        "srr2","srr3", "trasp","hli","saga_slp1","saga_slp2","saga_slp3",
                        "saga_tri1","saga_tri2","saga_tri3","saga_tpi","saga_proCrv1","saga_proCrv2", 
                        "saga_proCrv3","saga_plnCrv1","saga_plnCrv2","saga_plnCrv3","saga_lngCrv1",
                        "saga_lngCrv2","saga_lngCrv3","saga_csCrv1","saga_csCrv2","saga_csCrv3")
  ...
  if(tNum == 1){
    write.csv(pntsExt, paste0(pntDir, "pnts_ext.csv"))
  } else{
    PntsPre <- read.csv(paste0(pntDir, "pnts_ext.csv"))
    pntsExt <- bind_rows(pntsExt, PntsPre)
    write.csv(pntsExt, paste0(pntDir, "pnts_ext.csv"))
  }
}