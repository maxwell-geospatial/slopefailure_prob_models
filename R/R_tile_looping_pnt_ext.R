...
#Read in point data
pnts <- st_read("PATH TO YOUR SAMPLE POINTS")
...
pntDir = "OUTPUT DIRECTORY FOR POINTS"
...
for(tNum in 1:nrow(tiles)){
  ...
  #It is not necessary to save the stack to disk to extract the point values
  ...
  if(tNum == 1){
    write.csv(pntsExt, paste0(pntDir, "POINT OUTPUT"))
  } else{
    PntsPre <- read.csv(paste0(pntDir, "POINT OUTPUT"))
